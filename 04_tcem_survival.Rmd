...
vals = unique(gsub("_diff", "", gsub("_neo", "", gsub("_orig", "", names(tcemfreqs))))) #"aff_str"  "rp_str"   "and_str"  "or_str"   "aff_weak" "rp_weak"  "and_weak" "or_weak"
rm(tcemfreqs)
...
pat_ratio = lapply(ids, function(x) {
  ...
  a = lapply(tmn_list, function(y) {
    ...
    neo_ratio = sapply(neo_tcem, function(z) {
      ...
      keeppeps = !is.na(tcemexpr)|!is.na(tcemthymo)
      tcemfreq = tcemfreq[keeppeps]
      tcemexpr = tcemexpr[keeppeps]
      tcemthymo = tcemthymo[keeppeps]
      n1 = length(tcemfreq)
      ratios = c(n1, 
                 sum(tcemfreq < 4) / n1, 
                 sum(tcemexpr < 0.203075) / n1, 
                 sum(tcemthymo < 0.914470) / n1, 
                 sum(tcemfreq < 4 & tcemexpr < 0.203075) / n1, 
                 sum(tcemfreq < 4 & tcemthymo < 0.914470) / n1, 
                 sum(tcemexpr < 0.203075 & tcemthymo < 0.914470) / n1, 
                 sum(tcemfreq < 4 & tcemexpr < 0.203075 & tcemthymo < 0.914470) / n1)
      ratios
    })
    rownames(neo_ratio) = c("n_pep", "lowfreq", "lowexpr", "lowthymo", "lfreq_lexpr", "lfreq_lthymo", "lexpr_lthymo", "lflelt")
    neo_ratio
  })
  a = do.call(rbind,a)
  rownames(a) = paste(rep(vals, each = 8), rownames(a), sep = "_")
  a
})
pat_ratio[[1]]
pat_ratio = do.call(rbind, pat_ratio)
rownames(pat_ratio) = paste(rep(ids, each = 64), rownames(pat_ratio), sep = "_")
...
res %>% filter(obs.exp.dif.high>0, n.high>20, tcemgroup<0.05) %>% View()
