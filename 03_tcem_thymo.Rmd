...
lapply(ids, function(x) {
  ...
  save(tcemthymos, file = paste0(ext_folder1,"03out/tcem_mutated_nonamers_tcemthymos/", x))
})

#Drop patients without data
patdropped = ids[sapply(ids, function(x) {
  load(paste0(ext_folder1, "03out/tcem_mutated_nonamers_tcemthymos/", x))
  all(lengths(tcemthymos)==0)
})]

for(i in 1:length(patdropped)) {
  unlink(paste0(ext_folder1, "03out/tcem_mutated_nonamers_tcemthymos/", patdropped[i]))
} #8887
...
```{r}
ids = list.files(paste0(ext_folder1, "03out/tcem_mutated_nonamers_tcemthymos")) #8887
...
tumors_tcemthymos = lapply(tumors, function(x) {
  tempids = names(tt)[tt == x]
  tempids = tempids[tempids %in% ids]
  temp_tcemthymos = lapply(tempids, function(y) {
    load(paste0(ext_folder1, "03out/tcem_mutated_nonamers_tcemthymos/", y))
    md = c(aff_str_orig = median(tcemthymos$aff_str_orig, na.rm = T), 
           aff_str_neo = median(tcemthymos$aff_str_neo, na.rm = T), 
           aff_str_diff = median(tcemthymos$aff_str_diff, na.rm = T),
           rp_str_orig = median(tcemthymos$rp_str_orig, na.rm = T), 
           rp_str_neo = median(tcemthymos$rp_str_neo, na.rm = T), 
           rp_str_diff = median(tcemthymos$rp_str_diff, na.rm = T),
           or_str_orig = median(tcemthymos$or_str_orig, na.rm = T), 
           or_str_neo = median(tcemthymos$or_str_neo, na.rm = T), 
           or_str_diff = median(tcemthymos$or_str_diff, na.rm = T),
           and_str_orig = median(tcemthymos$and_str_orig, na.rm = T), 
           and_str_neo = median(tcemthymos$and_str_neo, na.rm = T), 
           and_str_diff = median(tcemthymos$and_str_diff, na.rm = T),
           aff_weak_orig = median(tcemthymos$aff_weak_orig, na.rm = T), 
           aff_weak_neo = median(tcemthymos$aff_weak_neo, na.rm = T), 
           aff_weak_diff = median(tcemthymos$aff_weak_diff, na.rm = T),
           rp_weak_orig = median(tcemthymos$rp_weak_orig, na.rm = T), 
           rp_weak_neo = median(tcemthymos$rp_weak_neo, na.rm = T),
           rp_weak_diff = median(tcemthymos$rp_weak_diff, na.rm = T),
           or_weak_orig = median(tcemthymos$or_weak_orig, na.rm = T), 
           or_weak_neo = median(tcemthymos$or_weak_neo, na.rm = T), 
           or_weak_diff = median(tcemthymos$or_weak_diff, na.rm = T),
           and_weak_orig = median(tcemthymos$and_weak_orig, na.rm = T),
           and_weak_neo = median(tcemthymos$and_weak_neo, na.rm = T),
           and_weak_diff = median(tcemthymos$and_weak_diff, na.rm = T))
    names(md) = value_names
    md
  })
  names(temp_tcemthymos) = tempids
  temp_tcemthymos = do.call(rbind,temp_tcemthymos)
})
names(tumors_tcemthymos) = tumors
tumors_tcemthymos$acc %>% View()
save(tumors_tcemthymos, file = paste0(ext_folder1,"03out/tumors_tcemthymos_all_mpi"))
```
...
tumors_tcemthymos = lapply(tumors, function(x) {
  ...
  temp_tcemthymos = lapply(tempids, function(y) {
    load(paste0(ext_folder1, "03out/tcem_mutated_nonamers_tcemthymos/", y))
    tcemthymos = lapply(tcemthymos, function(z) {
...
names(tumors_tcemthymos) = tumors
tumors_tcemthymos$acc %>% View()
save(tumors_tcemthymos, file = paste0(ext_folder1,"03out/tumors_tcemthymos_pass_mpi"))
```
...
tumors_tcemthymos = lapply(tumors, function(x) {
  ...
  temp_tcemthymos = lapply(tempids, function(y) {
    load(paste0(ext_folder1, "03out/tcem_mutated_nonamers_tcemthymos/", y))
    m = max(lengths(tcemthymos), na.rm = T)
    sapply(tcemthymos, function(p) fillnafun(p, m)) #create matrix
  })
  names(temp_tcemthymos) = tempids
  temp_tcemthymos = do.call(rbind,temp_tcemthymos)
})
names(tumors_tcemthymos) = tumors
save(tumors_tcemthymos, file = paste0(ext_folder1,"03out/tumors_tcemthymos_all_napi"))
```
...
names(tumors_tcemthymos) = tumors
tumors_tcemthymos$acc %>% View()
save(tumors_tcemthymos, file = paste0(ext_folder1,"03out/tumors_tcemthymos_pass_napi"))
```
...
bplot = function(x) {
  ...
  tempdf %<>% tidyr::gather(key = origvsneo, value = mediantcemexpr, 2:3)
  ...
  fig = ggplot(tempdf, aes(x = study, y = mediantcemexpr, fill = origvsneo)) + 
    ...
    scale_y_continuous(breaks = seq(.95,1,.01)) +
    geom_hline(yintercept = seq(.95,1,.01), linetype = "dashed") +    
    theme_bw() +
      theme(
...
bplot_diff = function(z) {
  ...
  fig = ggplot(tempdf, aes(x = study, y = diff)) + 
    geom_boxplot(outlier.shape = NA) + 
    scale_y_continuous(breaks = seq(-.01,.05,.01)) +
    geom_hline(yintercept = seq(-.01,.05,.01), linetype = "dashed") +    
    theme_bw() +
      theme(
        legend.position = "none",
        axis.title.x = element_text(size = 16, vjust = 0),
...
tcemthymos_files = grep("tumors_tcemthymos", list.files(paste0(ext_folder1,"03out/")),value = T)

for(i in tcemthymos_files) {
  id1 = paste0(strsplit(i,"_")[[1]][3:4], collapse = "_")
  load(paste0(ext_folder1,"03out/", i))
  res0 = matrix(ncol = (length(value_names)+1), dimnames = list(NA,c("tumor", value_names)))
  for(i in 1:length(tumors)) {
    res0 = rbind(res0, cbind.data.frame(tumor = tumors[i], tumors_tcemthymos[[tumors[i]]]))
  }
  rm(i)
  res0 = res0[-1,]
  ...
  for(j in grep("orig", value_names, value = T)) {
    ...
    bplot(x = j)
    ggsave(filename = paste0(ext_folder1,"03out/plots/", id1, "_", id2, ".jpg"), width = 35, height = 25, units = "cm", dpi = "retina")
  }
  #PLOT - diff
  for(k in grep("diff", value_names, value = T)) {
    id3 = paste(strsplit(k,"_")[[1]][1],strsplit(k,"_")[[1]][2],sep = "_")
    bplot_diff(z = k)
    ggsave(filename = paste0(ext_folder1,"03out/plots/", id1, "_", id3, "_diff.jpg"), width = 35, height = 25, units = "cm", dpi = "retina")
  }
}
...
```{r}
load(paste0(ext_folder1,"03out/tumors_tcemthymos_all_napi")) #OR
load(paste0(ext_folder1,"03out/tumors_tcemthymos_pass_napi"))

load(paste0(ext_folder1, "objects/value_names"))
load(paste0(ext_folder1, "objects/tumors"))
load(paste0(ext_folder1,"objects/study_abbrev"))

nonamer_count= sapply(tumors_tcemthymos, function(x) {
  x %<>% as.data.frame()
...
names(tcemthymos_samples) = gsub("_orig", "", grep("orig", value_names, value = T))
rm(nonamer_mins, nonamer_count)

for(i in names(tcemthymos_samples)) {
  tempdf = tcemthymos_samples[[i]]
  #ORIG vs NEO
  ...
  ggplot(tempdf_on, aes(x = study, y = value, fill = pepsource)) + 
    ...
    theme(
      ...
      panel.grid.minor = element_blank(),
      axis.line = element_line(colour = "black"))
  ggsave(filename = paste0(ext_folder1,"03out/plots/random_pass_",i,".jpg"), width = 35,height = 25,units = "cm",dpi = "retina")
  #DIFF
  ...
  ggplot(tempdf_dif, aes(x = study, y = value)) + 
    ...
    theme(
      ...
      panel.grid.minor = element_blank(),
      axis.line = element_line(colour = "black"))
  ggsave(filename = paste0(ext_folder1,"03out/plots/random_pass_diff_",i,".jpg"), width = 35,height = 25,units = "cm",dpi = "retina")
}
rm(tempdf,tempdf2,i)
```

#Ratio of neononamers with low TCEM thymoprotesome score (<4)
cutoff: 25%: 0.914470

```{r}
load(paste0(ext_folder1,"objects/study_abbrev"))
load(paste0(ext_folder1,"03out/tumors_tcemthymos_pass_napi"))
vals = unique(gsub("_diff", "", gsub("_neo", "", gsub("_orig", "", colnames(tumors_tcemthymos$acc)))))

neolowtcemthymo = sapply(tumors_tcemthymos, function(x) {
  ...
  sapply(vals, function(y) {
    ...
    ratio = nrow(subset(tempdf2, orig >= 0.914470 & neo < 0.914470)) / nrow(tempdf2)
    names(ratio) = y
    ratio
  })
})

neolowtcemthymo = as.data.frame(neolowtcemthymo)
neolowtcemthymo$bt = vals #binding_type
neolowtcemthymo = gather(data = neolowtcemthymo, key = "tumor", value = "ratio", -bt)

for(i in vals) {
  tempdf = neolowtcemthymo[neolowtcemthymo$bt == i,]
  ...
  ggsave(filename = paste0(ext_folder1,"03out/plots/neolowtcemthymo_",i,".jpg"), width = 25,height = 15,units = "cm",dpi = "retina")
}
