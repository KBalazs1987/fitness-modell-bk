...
res = expand.grid(tumor = c("all", freq_tumors), im.tumor.filt = c("all", "im_tumor"), survival.type = c("os", "dss", "dfi", "pfi"), loh = c("all", "notloh"), tcemv = tcemvars, tcemv_filter = c("no", "yes"), stringsAsFactors = F)
...
for(i in 1:nrow(res)) {
  tryCatch({  
    ...
    cohort_f$tcemgroup = cut(x = cohort_f$tcemvar, breaks = c(0,cutpoint,1), include.lowest = T, labels = F, right = F)
    ...
    # ggsurvplot(fit = survfit(Surv(time = OS_Time, event = OS_Event) ~ tcemgroup, data = cohort_f), pval = TRUE, legend.title = res$tcemv[i], surv.median.line = c("hv"), title = paste(c("Dataset:", res[i,1:6]), sep = " ", collapse = " "), font.main = 8)
...
ggforest(coxph(Surv(OS_Time, OS_Event) ~ a + clinical_GENDER + m + hz + p, data = cohort_f), data = cohort_f)

save(res, res_ftest, file = paste0(ext_folder1,"02out/survival_cutpoint40"))
```
