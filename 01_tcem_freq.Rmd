...
save(pat_neolowtcemfreq, file = paste0(ext_folder1,"01out/patients_neolowtcemfreq"))
```

#INNEN FOLYT#Survival

```{r}
load(paste0(ext_folder1,"01out/patients_neolowtcemfreq"))
ids = list.files(paste0(ext_folder1, "clinical")) #8943
patsurv = t(sapply(ids, function(x) {
  load(paste0(ext_folder1, "clinical/", x))
  clinical[c("clinical_AGE", "clinical_GENDER", "clinical_OS", "clinical_OS.time", "clinical_DSS", "clinical_DSS.time", "clinical_DFI", "clinical_DFI.time", "clinical_PFI", "clinical_PFI.time")]
}))

data = cbind.data.frame(pat_neolowtcemfreq, patsurv[match(rownames(pat_neolowtcemfreq), rownames(patsurv)),])
```
