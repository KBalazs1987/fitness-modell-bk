...
ids = list.files(paste0(ext_folder1, "clinical")) #10967

no_cores <- detectCores() - 1
cl <- makeCluster(no_cores)
#Load clinical data
clinical = parSapply(cl, list.files("d:/CloudStation/fitness-model-ext/clinical"), function(x) {
  load(paste0("d:/CloudStation/fitness-model-ext/clinical/", x))
  return(clinical)
})
stopCluster(cl)
rm(cl, no_cores)
...
#"clinical_HISTORY_NEOADJUVANT_TRTYN" = No

data = cbind.data.frame(pat_neolowtcemfreq, clinical[match(rownames(pat_neolowtcemfreq), clinical$SampleID),])
