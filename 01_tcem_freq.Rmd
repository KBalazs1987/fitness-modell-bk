...
##INNENTŐL JAVÍTANI!!!!!!!!!!!!!!!!!!!!!!!!!Passenger mutations - median per individual - 
...
for(i in names(tcemfreqs_samples)) {
  ...
  tempdf2 = tempdf[tempdf$pepsource == "neo",]
  tempdf2$study = fct_reorder(.f = tempdf2$study, .x = tempdf2$value)
  tempdf$study = factor(tempdf$study, levels = levels(tempdf2$study))
  ggplot(tempdf, aes(x = study, y = value, fill = pepsource)) + 
    ...
    theme(
      ...
      panel.grid.minor = element_blank(),
      axis.line = element_line(colour = "black"))
  ggsave(filename = paste0(ext_folder1,"01out/plots/random_all_",i,".jpg"), width = 35,height = 25,units = "cm",dpi = "retina")
}
rm(tempdf,tempdf2,i)
```
...
for(i in diff_names) {
  bplot(x = i, plog = F)
  ggsave(filename = paste0(ext_folder1,"01out/plots/diffs/pass_napi_", i, ".jpg"), width = 20, height = 20, units = "cm", dpi = "retina")
}
